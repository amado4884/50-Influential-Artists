...
function removeArtist(array, index) {
  return array.filter((array, i) => i !== index);
}
...
function lotsOfArt(artists) {
  ...
  return artists.filter(artist => artist.paintings > 100).map(artist => artist.name);
...
function get20s(artists) {
  ...
  const twenties = artists.filter(artist => {
    const year = Number(artist.years.split(" ")[0]);
  ...
  return twenties.map(artist => artist.name);
...
const artistsElement = document.getElementById("artists-container");
function getHTML(data) {
  let response = "";
  data.forEach(artist => {
    response = response + `<div class="artist">
    <div class="image">
        <img src="https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg"/>
    </div>
    <div class = "name">
...
// artistsElement.innerHTML = getHTML(artists);
